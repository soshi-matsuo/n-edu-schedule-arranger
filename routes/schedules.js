...
router.get('/new', authenticateEnsurer, (req, res, next) => {
    res.render('new', { user: req.user });
});

router.post('/', authenticateEnsurer, (req, res, next) => {
...
router.get('/:scheduleId', authenticateEnsurer, (req, res, next) => {
...
router.get('/:scheduleId/edit', authenticateEnsurer, (req, res, next) => {
    Schedule.findOne({ where: { scheduleId: req.params.scheduleId } })
        .then((schedule) => {
            if (isMine(req, schedule)) {
                ...
                const err = new Error('指定された予定がないか、編集する権限がありません');
                err.status = 404;
                next(err);
            }
        });
...
router.post('/:scheduleId', authenticateEnsurer, (req, res, next) => {
    Schedule.findOne({
        ...
        if (schedule && isMine(req, schedule)) {
            if (parseInt(req.query.edit) === 1) {
                ...
                deteleScheduleAggregate(req.params.scheduleId, () => {
                    res.redirect('/');
                });
            } else {
...
const parseCandidateNames = req => req.body.candidates.trim().split('\n').map(s => s.trim()).filter(s => s !== '');

const isMine = (req, schedule) => {
    return schedule && parseInt(req.user.id) === parseInt(schedule.createdBy);
};

const deteleScheduleAggregate = (scheduleId, done, err) => {
